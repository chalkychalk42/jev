...
def generate(db: sqlite3.Connection, profiles: dict, guide_quests: set[int] | None = None) -> dict:
    ...
    # White and green gear, gems, trade goods and recipes that no quest asks for or hands over:
    ...
    # Only the quests the guides walk (V241): a trade good some other quest somewhere asks
    # for was never sold, and the level 8 mage's bags filled with a Tigerseye (1 silver),
    # Malachite, Linen Cloth and Small Spider Legs, "no junk" at each merchant while it
    # could not pay for Frostbolt (session 212). Without the guides' quests, every quest.
    if guide_quests is None:
        rows = db.execute("select " + ",".join(required_cols) + " from world_quest_template")
    else:
        rows = (row[1:] for row in db.execute(
            "select entry," + ",".join(required_cols) + " from world_quest_template")
            if row[0] in guide_quests)
    required = {int(i) for row in rows for i in row if i}
    # Gems too (class 3): a Tigerseye sells for a silver and a Moss Agate for four.
    surplus = {str(row[0]): int(row[1]) for row in db.execute(
        "select entry,SellPrice from world_item_template where class in (2,3,4,7,9) "
...
def guide_quests(folder: pathlib.Path) -> set[int]:
    """Every quest the generated guides in `folder` step through."""
    found: set[int] = set()
    for path in sorted(folder.glob("*.json")):
        ...
        if isinstance(graph, dict) and isinstance(graph.get("nodes"), list):
            found |= {n["quest_id"] for n in graph["nodes"]
                      if isinstance(n, dict) and isinstance(n.get("quest_id"), int)}
    return found
...
def main() -> int:
    ...
    with sqlite3.connect(f"file:{args.db}?mode=ro", uri=True) as db:
        catalog = generate(db, profiles, guide_quests(ROOT / "content/tbc"))
