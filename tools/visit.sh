...
#   tools/visit.sh NAME LOG  # e.g. tools/visit.sh Itheamar captures/live-mage-check-2.log
#   rm var/loop/hold
...
say "one session on $name"
WSLENV="${WSLENV:+$WSLENV:}JEV_OPERATOR_QUIET_S" JEV_OPERATOR_QUIET_S=600 \
  timeout 1080 "$WINPY" -u tools/start_teaching.py --run --dispatch hybrid > "$log" 2>&1
say "session exit=$?"
say "back to $back"
